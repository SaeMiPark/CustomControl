...
sap.ui.define([
	...
	function(library, Element, Renderer, coreLibrary, Device, InvisibleText, Sorter) {
	"use strict";
	...
	var Column = Element.extend("sap.m.Column", /** @lends sap.m.Column.prototype */ { metadata : {
		...
		properties : {
			...
			/**
			 * 정렬할 때 사용할 Path에 대해 사용자에게 입력받기 위해 추가
			 */
			sortProperty: {type: "string", group: "Behavior", defaultValue: null},
			/**
			 * 필터를 사용할 Path에 대해 사용자에게 입력받기 위해 추가 -> ComboBox 데이터 바인딩용
			 */
	...
	Column.prototype.ontap = function(oEvent) {
		...
		/**
		 * 컬럼을 클릭 할 때 sortProperty가 존재시 테이블이 정렬되게 구현
		 */
		if(this.getProperty("sortProperty") != ''){
			const oColumns = this.getTable().getColumns();
			const oldIndicator = this.getSortIndicator();
			const oBinding = this.getTable().getBinding("items");

			let aSorters = null;

			for(const column of oColumns){
				column.setSortIndicator("None");
			};

			if(oldIndicator == 'None'){
				// None > Ascending
				this.setSortIndicator("Ascending");
				aSorters = new Sorter(this.getProperty("sortProperty"), false);
			}
			else if(oldIndicator == 'Ascending'){
				// Ascending > Descending
				this.setSortIndicator("Descending");
				aSorters = new Sorter(this.getProperty("sortProperty"), true);
			}
			else{
				// Descending > None
	...
	Column.prototype.setSortIndicator = function(sSortIndicator) {
		...
		if(this.getProperty("sortProperty") != ''){
			switch(sSortIndicator){
				case 'None':
					this.getAggregation("header").removeStyleClass("customSortAsc");
					this.getAggregation("header").removeStyleClass("customSortDesc");

					this.getAggregation("header").addStyleClass("customSortNone");
					break;
				case 'Ascending' :
					this.getAggregation("header").removeStyleClass("customSortNone");
					this.getAggregation("header").removeStyleClass("customSortDesc");

					this.getAggregation("header").addStyleClass("customSortAsc");
					break;
				...
				case 'Descending':
					...
					this.getAggregation("header").addStyleClass("customSortDesc");
					break;
				default:
					break;
			};
	...
	Column.prototype.setHeader = function (oControl) {
		...
		//클릭전에도 이미지 보여야함
		if(this.getProperty("sortProperty") != ''){
			oNewHeader.addStyleClass("customSortNone") 
		}

		return this;
	};

	// Column.prototype.setIcon = function (oControl) {
	// 	return this;
	// };
